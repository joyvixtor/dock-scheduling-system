...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "OutboundDock":
		resolverName, err := entityResolverNameForOutboundDock(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "OutboundDock": %w`, err)
		}
		switch resolverName {
		...
		case "findOutboundDockByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findOutboundDockByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindOutboundDockByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "OutboundDock": %w`, err)
			}
...
func entityResolverNameForOutboundDock(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for OutboundDock", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for OutboundDock", ErrTypeNotFound))
			break
		}
		return "findOutboundDockByID", nil
	}
	return "", fmt.Errorf("%w for OutboundDock due to %v", ErrTypeNotFound,
