...
func (ec *executionContext) resolveEntity(
	...
	case "TransferTask":
		resolverName, err := entityResolverNameForTransferTask(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "TransferTask": %w`, err)
		}
		switch resolverName {
		...
		case "findTransferTaskByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findTransferTaskByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindTransferTaskByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "TransferTask": %w`, err)
			}
...
func entityResolverNameForTransferTask(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for TransferTask", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for TransferTask", ErrTypeNotFound))
			break
		}
		return "findTransferTaskByID", nil
	}
	return "", fmt.Errorf("%w for TransferTask due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}
