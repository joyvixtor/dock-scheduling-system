...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "InboundDock":
		resolverName, err := entityResolverNameForInboundDock(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "InboundDock": %w`, err)
		}
		switch resolverName {
		...
		case "findInboundDockByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findInboundDockByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindInboundDockByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "InboundDock": %w`, err)
			}
...
func entityResolverNameForInboundDock(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for InboundDock", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for InboundDock", ErrTypeNotFound))
			break
		}
		return "findInboundDockByID", nil
	}
	return "", fmt.Errorf("%w for InboundDock due to %v", ErrTypeNotFound,
